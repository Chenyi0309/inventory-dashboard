...
# ============== 写入自检 & 回读校验（放在保存逻辑之后） ==============
with st.expander("🛠 写入自检 / 回读校验", expanded=False):
    # 1) 目标 URL & 服务账号
    st.write("🔗 当前 Sheet URL:", os.getenv("INVENTORY_SHEET_URL") or st.secrets.get("INVENTORY_SHEET_URL", "(未配置)"))
    if "service_account" in st.secrets:
        st.write("🔑 Service Account:", st.secrets["service_account"].get("client_email", "(no client_email in secrets)"))
    else:
        st.warning("secrets 中没有 service_account，无法以服务账号写入。")

    # 2) 列出工作表、显示'购入/剩余'表头
    try:
        from gsheet import _debug_list_sheets, _debug_read_header
    except Exception:
        _debug_list_sheets = _debug_read_header = None

    if _debug_list_sheets:
        try:
            st.write("📄 工作表列表：", _debug_list_sheets())
        except Exception as e:
            st.error(f"列出工作表失败：{e}")

    if _debug_read_header:
        try:
            st.write("📑 『购入/剩余』表头：", _debug_read_header())
        except Exception as e:
            st.error(f"读取表头失败：{e}")
    ...
    try:
        ...
        df_chk = read_records_fn()
        # 统一列名，避免看起来“不在同一表头”
        try:
            df_chk = normalize_columns_compute(df_chk)
        except:
            pass
        cols = [c for c in ["日期 (Date)","食材名称 (Item Name)","分类 (Category)","数量 (Qty)","单位 (Unit)",
                            "单价 (Unit Price)","总价 (Total Cost)","状态 (Status)","备注 (Notes)"] if c in df_chk.columns]
        st.caption("回读：『购入/剩余』表尾 10 行")
        st.dataframe(df_chk.tail(10)[cols], use_container_width=True)
    except Exception as e:
        st.error(f"回读失败：{e}")
