...
with tabs[1]:
    ...
    # ========= 关键：强力清洗 & 兜底恢复『分类 (Category)』 =========
    # 字段统一 strip；空串→NaN
    for c in ["食材名称 (Item Name)", "分类 (Category)", "状态 (Status)", "单位 (Unit)", "备注 (Notes)"]:
        if c in df.columns:
            df[c] = df[c].astype(str).str.strip()
    if "分类 (Category)" in df.columns:
        df["分类 (Category)"] = df["分类 (Category)"].replace({"": np.nan, "None": np.nan, "nan": np.nan})

        # 若整列为空，尝试别名列
        if df["分类 (Category)"].isna().all():
            for alt in ["分类", "Category"]:
                if alt in df.columns:
                    df["分类 (Category)"] = df["分类 (Category)"].fillna(df[alt].astype(str).str.strip())
            df["分类 (Category)"] = df["分类 (Category)"].replace({"": np.nan})

        # 仍有缺失 → 按物品名继承“最近一次非空分类”
        if df["分类 (Category)"].isna().any():
            try:
                latest_cat = (
                ...
                def _fill_cat(row):
                    if pd.notna(row["分类 (Category)"]):
                        return row["分类 (Category)"]
                    return latest_cat.get(row["食材名称 (Item Name)"], np.nan)
                df["分类 (Category)"] = df.apply(_fill_cat, axis=1)
            except Exception:
                pass
    ...
    # 类别列表（来自清洗后的 DataFrame）
    all_types = sorted(df["分类 (Category)"].dropna().unique().tolist()) if ("分类 (Category)" in df.columns and not df.empty) else []
